...
function parseRoom() {
  ...
  return normalizeRoomName(
    params.get("room") || localStorage.getItem("pendingDevRoom") || "",
  );
}

async function fetchRoomSnapshot(room) {
  const response = await fetch(
    `${SERVER_ORIGIN}/api/dev/rooms/${encodeURIComponent(room)}`,
  );
  if (!response.ok) {
...
function goToHeroSelect(room, team, nickname, mode) {
  ...
  localStorage.setItem("pendingDevTeam", team);
  ...
  window.location.href = `${FRONTEND_ORIGIN}/dev/hero-select.html?${params.toString()}`;
}
