...
function getParams() {
  ...
  const room = normalizeRoomName(
    p.get("room") || localStorage.getItem("pendingMultiRoom") || "",
  );
  const team =
    (
      p.get("team") ||
      localStorage.getItem("pendingMultiTeam") ||
      "red"
    ).toLowerCase() === "blue"
      ? "blue"
      : "red";
...
function goToGame(params) {
  ...
  window.location.href = `${FRONTEND_ORIGIN}/multi/game.html?${query.toString()}`;
}
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  function persistPendingSelection() {
    ...
    localStorage.setItem("pendingMultiRoom", params.room);
    localStorage.setItem("pendingMultiTeam", params.team);
    localStorage.setItem("pendingMultiMode", params.roomMode);
  }
  ...
  socket.on("roomError", (payload) => {
    ...
    if (code === "ROOM_NOT_FOUND") {
      statusLine.textContent =
        "Room introuvable (serveur redemarre). Retourne au lobby pour la recreer.";
      return;
    }
    if (code === "ROOM_MODE_MISMATCH") {
      statusLine.textContent =
        "Mode de room incorrect. Retourne au lobby puis reessaie.";
      return;
    }
    statusLine.textContent = `Erreur room: ${code}`;
  });

  if (startGameBtn) {
    startGameBtn.addEventListener("click", () => {
      if (!isOwner || roomStarted) return;
      socket.emit("startRoom", (ack) => {
        if (ack && ack.ok === false) {
          if (ack.code === "NOT_LEADER") {
            statusLine.textContent = "Tu n'es plus chef de room.";
            return;
          }
          statusLine.textContent = `Erreur lancement: ${ack.code || "UNKNOWN"}`;
          return;
        }
  ...
  backTeamBtn.addEventListener("click", () => {
    ...
    window.location.href = `${FRONTEND_ORIGIN}/multi/team.html?${q.toString()}`;
  });
});
